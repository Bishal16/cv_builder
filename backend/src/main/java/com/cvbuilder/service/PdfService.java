...
public class PdfService {
    ...
    private String generateHtml(Cv cv) {
        ...
        html.append(".rich-text li { margin-bottom: 3px; }");
    ...
    private void appendClassicStyles(StringBuilder html) {
        html.append(".page { padding: 0; }");
        html.append(".header { border-bottom: 2px solid #333; padding: 10mm 12mm 8mm 12mm; margin-bottom: 0; }");
    ...
    private void generateClassicHtml(StringBuilder html, Cv cv) {
        ...
        html.append("<div class='contact-info wrap-anywhere'>")
        ...
        // Sidebar
        html.append("<div class='sidebar'><div class='sidebar-inner'>");
        ...
        // Main
        html.append("<div class='main-content'><div class='main-content-inner'>");
        if (info.getSummary() != null && !info.getSummary().isEmpty()) {
            ...
            html.append("<div class='rich-text'>").append(preserveHyphenLineBreaks(info.getSummary())).append("</div>");
        }
        ...
        if (cv.getProjects() != null && !cv.getProjects().isEmpty()) {
            ...
            for (Project p : cv.getProjects()) {
                ...
                if (p.getUrl() != null && !p.getUrl().isEmpty()) {
                    html.append("<div class='wrap-anywhere' style='color: #3182ce; font-size: 9pt;'>").append(escapeHtml(p.getUrl())).append("</div>");
                }
                html.append("<div class='mt-1 rich-text'>").append(preserveHyphenLineBreaks(p.getDescription())).append("</div>");
                html.append("</div>");
            }
        }
    ...
    private void appendModernStyles(StringBuilder html) {
        html.append(".modern-header { background-color: #2563eb; color: white; padding: 10mm 12mm; margin-bottom: 6mm; }");
        ...
        html.append(".modern-body { padding: 0 12mm 12mm 12mm; }");
    ...
    private void generateModernHtml(StringBuilder html, Cv cv) {
        ...
        html.append("<div class='modern-contact wrap-anywhere'>").append(escapeHtml(info.getEmail())).append(" &bull; ").append(escapeHtml(info.getPhone())).append(" &bull; ").append(escapeHtml(info.getLocation())).append("</div>");
        html.append("</div>");

        html.append("<div class='modern-body'>");
        if (info.getSummary() != null && !info.getSummary().isEmpty()) {
            html.append("<div class='modern-section-title' style='margin-top:0;'>About Me</div>");
            html.append("<div class='modern-card rich-text'>").append(preserveHyphenLineBreaks(info.getSummary())).append("</div>");
        }
    ...
    private void generateAtsHtml(StringBuilder html, Cv cv) {
        ...
        html.append("<div class='wrap-anywhere'>").append(escapeHtml(info.getEmail())).append(" | ").append(escapeHtml(info.getPhone())).append(" | ").append(escapeHtml(info.getLocation())).append("</div>");
        html.append("</div>");
    ...
    private String escapeHtml(String input) {
        ...
        return preserveHyphenLineBreaks(escaped);
    }

    private String preserveHyphenLineBreaks(String content) {
        if (content == null || content.isEmpty()) {
            return "";
        }

        return content.replaceAll("(?<=\\p{L}|\\p{N})-(?=\\p{L}|\\p{N})", "&#8209;");
    }
}
