...
- `src/templates/` contains A4-proportioned preview components: `ClassicTemplate`, `ModernTemplate`, `AtsTemplate`, `ProTemplate`, `MinimalTemplate`. `CvPreview.tsx` picks the right one based on `templateId`. `CvPrintView.tsx` is the full-page version Playwright loads.

### Backend request flow
`AuthController` / `CvController` / `PdfController` → `Service` layer → `CvRepository` (Spring Data JPA, PostgreSQL).
...
### Template alignment
`PdfService` uses Playwright to open `CvPrintView` (the frontend print view) in a headless Chromium browser, so the frontend templates (`src/templates/*Template.tsx`) are the PDF templates — there is no parallel Java HTML implementation. CSS or layout changes to a preview template are automatically reflected in exported PDFs.
...
- Use `@Column(columnDefinition = "TEXT")` on HTML content fields in JPA entities to avoid truncation in PostgreSQL.
...
### Template IDs
`templateId` is an enum: `CLASSIC`, `MODERN`, `ATS`, `PRO`, `MINIMAL`. Never use raw string literals for template IDs.
