...
/**
 * Build the OAuth start URL, passing the current origin so the backend can
 * redirect back to wherever the user is browsing (localhost, LAN IP, prod)
 * instead of the statically configured frontend URL.
 */
function oauthUrl(provider: 'google' | 'github'): string {
  return `/oauth2/authorization/${provider}?frontend_origin=${encodeURIComponent(window.location.origin)}`;
}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '16px 24px 48px', position: 'relative' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <OAuthBtn onClick={() => { window.location.href = oauthUrl('google'); }} label="Google">
            ...
            <OAuthBtn onClick={() => { window.location.href = oauthUrl('github'); }} label="GitHub">
